...
export function useNotifications() {
  ...
  useEffect(() => {
    ...
    // Temporary debug logs: confirm subscription and connection state
    try {
      console.log('🔔 Subscribing to Pusher channel:', `user-${user.id}`);
      console.log('🔌 Pusher initial connection state:', pusher.connection?.state);
      channel.bind('pusher:subscription_succeeded', () => {
        console.log('✅ Pusher subscription succeeded for channel:', `user-${user.id}`);
      });
      channel.bind('pusher:subscription_error', (err: any) => {
        console.warn('❌ Pusher subscription error for channel', `user-${user.id}`, err);
      });
      ...
      console.error('Failed to attach pusher debug listeners:', e);
    }
